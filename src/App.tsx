...
type PlayerMode = 'trailer' | 'film' | null;
...
function FilmCard({
  ...
  mode: Exclude<PlayerMode, null>;
  label: string;
  title: string;
  runtime: string;
  featured?: boolean;
  onOpen: (mode: Exclude<PlayerMode, null>) => void;
}) {
...
function SectionDivider({
  ...
  next: string;
  tone?: 'dark' | 'to-light' | 'to-dark' | 'to-red';
}) {
  return (
...
export default function Home() {
  ...
  const scrollSoundTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const fadeAudio = useCallback((target: number, duration = 700, pauseAfter = false) => {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => event.key === 'Escape' && setPlayerMode(null);
  ...
  return (
    <main className="site" style={{ '--scroll-progress': scrollProgress } as CSSProperties}>
